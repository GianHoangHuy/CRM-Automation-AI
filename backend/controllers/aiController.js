...
const Order = require("../models/Order"); // Bắt buộc thêm model Order để tính doanh thu
...
const handleStatRevenue = async (timeFrame) => {
    try {
        ...
        // Lấy các đơn đã giao thành công (delivered)
        const orders = await Order.find({
            createdAt: { $gte: startDate },
            status: "delivered"
        }).lean();

        // Dùng totalPrice thay vì totalAmount cho đúng Schema của bạn
...
const handlePredictTrend = async () => {
    return {
        type: "admin_predict",
        message: "Dựa trên dữ liệu bán hàng, phân khúc Laptop Văn phòng đang có xu hướng tăng trưởng ổn định. Dự báo tháng tới nhu cầu các dòng máy mỏng nhẹ (Dell XPS, HP Envy) sẽ tăng khoảng 15%.",
...
const handleTopSellingProducts = async () => {
    try {
        ...
        // (Nếu Database chưa có cột soldCount thì nó sẽ lấy 5 cái đầu tiên, không lo lỗi sập web)
        const topProducts = await Product.find().sort({ soldCount: -1 }).limit(5);
        return {
            type: "form", // Trả về form để Frontend TỰ ĐỘNG BẬT BẢNG SẢN PHẨM BÊN TRÁI lên
            message: "Dạ đây là Top 5 sản phẩm đang bán chạy nhất hệ thống hiện tại, anh/chị xem bảng bên trái nhé:",
            products: topProducts
        };
    } catch (error) {
        return { type: "text", message: "Dạ có lỗi khi truy xuất danh sách sản phẩm." };
    }
};
